...
- Individual plan: $19/mo or $15.20/mo billed annually ($182.40/yr), unlocks everything above, unlimited posts. LAUNCH50 promo code gives 50% off the first 3 months. Includes a 7-day free trial. Cancel anytime. Cancellation takes effect at the end of the current billing period, and access continues until then.
...
export function parseAriaReply(raw: string): { text: string; action: AriaAction | null } {
  ...
  if (lastLine.startsWith('{') && lastLine.endsWith('}')) {
    ...
    } catch { /* not valid JSON, so treat as plain text */ }
  }
